...
let re = 'hello'; // CaseSetive
    ...
    //---------------------------------Shorthand Character:check in full string.... and Assertion:condition  ----------------------------------

    re = /\w/; // word character => string(latter+number) or '_'/'underscore';
    re = /\w+/; // one or more->  word character => string(latter+number) or '_'/'underscore';
    re = /\W/; //Non word character; match for special simbol '!@#$%^&*()_+' etc character;
    re = /\W+/; //one or more-> Non word character; match for special simbol '!@#$%^&*()_+' etc character;

    re = /\d/; //Digit [0-9]
    re = /\d+/; //one or more -> Digit [0-9]
    re = /\D/; //non Digit
    re = /\D+/; // one or more -> non Digit

    re = /\s/; // space;
    re = /\s+/; //one or more ->  space;
    re = /\S/; //  Nonspace;
    re = /\S+/; //one or more ->  Nonspace;

    re = /hello\b/ // word boundery =  "'hello' .... "
    re = /\bhello\b/ // word boundery =  "  'hello' .... "

    //------------------------------Assertion: conditon--------------------

    re = /x(?=yz)/; // match when 'x' before yz.
    re = /x(?!yz)/; // match when 'x' not before yz.

    let str;//----------------
   

   str = 'asdf asdfhello x yz ';
